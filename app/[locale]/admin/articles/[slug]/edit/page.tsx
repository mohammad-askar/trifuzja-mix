...
function normalize(api: ArticleFromApi, loc: Locale): ArticleEditable {
  if (!api.slug) {
    throw new Error('Article payload missing slug');
  }
  ...
  const status: ArticleStatus =
    api.status === 'draft' || api.status === 'published'
      ? api.status
      : 'draft';
    ...
    <main className="max-w-5xl mx-auto px-4 py-10 space-y-6">
      ...
      <ArticleEditor
        ...
        defaultData={{
          ...
          excerpt: toLocaleRecord(state.article.description),
          content: toLocaleRecord(state.article.contentHtml),
          page: state.article.pageKey,
