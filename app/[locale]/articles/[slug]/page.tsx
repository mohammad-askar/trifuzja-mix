...
// ✅ SSR-safe date formatting (ثابت بين السيرفر والعميل)
    ...
    transformTags: {
      a: (_tagName, attribs) => {
        ...
        if (out.target === "_blank") {
          const rel = (out.rel ?? "").toLowerCase();
          const parts = new Set(rel.split(/\s+/).filter(Boolean));
          parts.add("noopener");
          parts.add("noreferrer");
          out.rel = Array.from(parts).join(" ");
        }
...
export default async function ArticlePage(props: {
  ...
  const bodySafe = preSanitized ? sanitizeArticleHtml(preSanitized) : "";

  const created = art.createdAt ? new Date(art.createdAt) : new Date();
  const updated = art.updatedAt ? new Date(art.updatedAt) : created;

  // ✅ بدل toLocaleDateString (يسبب mismatch)
  const dateStr = formatDateStable(created);
        ...
        <div className="px-6 md:px-4 py-3 prose dark:prose-invert max-w-none">
          <header className="mb-1">
            ...
            <div className="mt-2 flex flex-wrap gap-4 text-sm text-gray-600 dark:text-gray-400">
              <time dateTime={created.toISOString()}>{dateStr}</time>
